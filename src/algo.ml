  ...
  *)
  ...
  let find_path2 (grDC: id graph) (id1:id) (id2:id) =
    ...
    let next_nodes = List.filter_map (fun arc ->
          if arc.lbl > 0 && not (List.mem arc.tgt visited) then Some arc.tgt
          else None
        ) out in
  ...
  |_::[]-> gr
    | x::y::rest ->  
      ...
      | Some(arc) ->
          ...
          (* Augmenter la capacité de l'arc inverse *)
          let rev_arc = find_arc_in_path gr y x in
          let gr = match rev_arc with
            | None -> add_arc gr y x valeur
            | Some(rev) -> add_arc gr y x (rev.lbl + valeur)
          in
